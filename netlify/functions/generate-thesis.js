...
exports.handler = async (event) => {
  ...
  try {
    // 1. UBAH INPUT: Kita sekarang menerima 'subChapterTitle'
    const { topic, problem, subChapterTitle } = JSON.parse(event.body);
    ...
    // 2. UBAH INSTRUKSI: Lebih fokus untuk menguraikan satu sub-bab
...
- Jika diminta membuat tabel (misalnya untuk Orisinalitas), buatlah dalam format Markdown.`;

    // 3. UBAH PROMPT: Jauh lebih sederhana dan langsung ke tujuan
...
Tolong tuliskan uraian lengkap untuk sub-bab berikut: "${subChapterTitle}"`;
    ...
    const requestBody = {
      ...
      generationConfig: {
        temperature: 0.7,
        topP: 0.9,
        maxOutputTokens: 8192, // Tetap besar untuk hasil yang panjang
      }
    };
    ...
    if (!responseData.candidates || responseData.candidates.length === 0) {
      const blockReason = responseData.promptFeedback?.blockReason || 'Tidak ada kandidat (mungkin diblokir atau terjadi error sementara)';
    ...
    // 4. PRE-PROCESSING OUTPUT: Tambahkan judul sub-bab di awal hasil
